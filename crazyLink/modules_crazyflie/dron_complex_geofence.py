...
# The watchdog as the simple geofence, enables to check periodically the position of the drone and it case of,
# move the drone.
def setComplexScenario (self,inside_polygon, exclusion, watchdog=True, blocking=True, callback=None, params = None):
    ...
    if blocking and watchdog==False:
        ...
        self._setComplexScenario(inside_polygon, exclusion, watchdog, callback, params)
        return True
    else:
        logging.info(f"[Dron] Geofence complejo creado.")
        self.checkcomplexGeofence = True
        self.complexGeofence = None
        scenarioThread = threading.Thread(target=self._setComplexScenario, args=[inside_polygon, exclusion, watchdog, callback, params])
...
def _setComplexScenario(self, inside_polygon=None, exclusion=None, watchdog=True, callback=None, params=None):
    ...
    if watchdog:
        while self.checkcomplexGeofence:
            self._moveComplexScenario(callback, params)
            time.sleep(0.3)
    else:
        self._moveComplexScenario(callback, params)
        time.sleep(0.3)
...
def _moveComplexScenario(self, callback=None, params = None):
    ...
    if self.state == "flying" and self.checkcomplexGeofence == True:
        ...
        if not zona_segura.contains(p):
            ...
            # If the distance is greater than 25 cm, act, otherwise it doesn't (just a buffer zone) (it can be changed)
            if dist < 0.25:
